...
const faqs = [
    ...
    ["1인창업가도 아이디어만으로 앱제작을 상담할 수 있나요?","가능합니다. 만들고 싶은 서비스의 사용자, 해결하려는 문제, 핵심 기능부터 함께 정리합니다. NERO는 요구사항 정의와 Figma 화면설계 후 웹·앱, 서버·DB, 관리자, 배포 범위를 제안하며, 초기 검증에 필요한 MVP/PoC부터 범위를 좁힐 수 있습니다."],
    ["정부지원사업이나 모두의창업을 준비할 때 MVP 제작을 의뢰할 수 있나요?","개발 상담은 가능합니다. NERO는 지원사업과 초기 검증에 필요한 MVP/PoC, 관리자, 전환 측정, 웹·앱 배포 범위를 함께 정리합니다. 모두의창업 수행 이력은 없으며, 개발 상담 가능 여부가 해당 사업의 선정이나 사업비 집행 가능성을 의미하지는 않습니다. 참여·집행 조건은 해당 사업의 공고와 운영기관을 통해 별도로 확인해 주세요."],
    ["MVP 외주개발은 어디까지 만들고 무엇을 검수하나요?","먼저 핵심 사용자 흐름을 정하고 인증, DB/API, 시연 데이터, 관리자, 배포 URL 중 필요한 범위를 선택합니다. NERO의 공개 MVP/PoC 산출물에는 Figma, MVP 웹앱, 관리자, QA 체크리스트가 안내되어 있습니다. 실제 포함 기능과 제외 기능, 사용자·관리자 검수 시나리오는 과업범위서에서 확정합니다."],
    ["앱제작을 맡기면 서버와 관리자, iOS·Android 배포도 포함되나요?","앱 화면뿐 아니라 서버, DB/API, 관리자, 앱 빌드, 스토어 등록 자료와 심사 대응까지 함께 상담할 수 있습니다. NERO는 iOS·Android 앱서비스와 운영형 웹서비스를 개발 범위로 안내하고 있습니다. 실제 포함 항목은 프로젝트별로 정하며 스토어 승인이나 심사 기간을 보장하지는 않습니다."],
    ["앱외주개발 비용과 일정은 어떤 기준으로 정하나요?","사용자 역할과 화면, 회원·결제·알림 같은 기능, 관리자 업무, 외부 API, 검수·배포 범위를 먼저 확인합니다. NERO는 요구사항과 기능정의서, 과업범위서를 정리한 뒤 우선순위에 따라 견적을 확정합니다. 일정과 산출물, 검수 기준은 계약·착수 단계에서 합의하므로 아이디어만으로 확정 가격이나 납기를 약속하지 않습니다."],
    ["개발 완료 후 소스코드와 운영 자료는 어떻게 인수인계하나요?","NERO는 소스코드와 함께 Git, ERD, API 기준, 서버 정보, 운영 매뉴얼 등 다음 담당자가 이어받는 데 필요한 자료를 정리하는 방식으로 개발합니다. 요구사항·과업범위서, Figma, QA 체크리스트, 배포·운영 문서 중 제공할 항목을 프로젝트 범위에 맞춰 확인합니다. 저장소와 서버·앱스토어 계정의 명의, 접근 권한, 이전 범위도 계약 전에 함께 정리해 주세요."],
    ["앱·웹 외주개발 경험은 어떤 공개 사례로 확인할 수 있나요?","이 페이지의 포트폴리오에서 프로젝트별 개발 범위를 확인할 수 있습니다. Nero 정신건강 통합관리 플랫폼은 모바일 기록, 건강 데이터 연동, 관리자, AI 서버와 앱 배포를 소개합니다. 소살리토 공식 온라인 사이트는 결제·알림·배송 API·운영 관리자, 소프티 Softie는 iOS/Android 앱 화면·서버·관리자·스토어 자료·심사 대응을 소개합니다. 상담에서는 원하는 기능과 공개 사례의 해당 범위를 비교해 주세요."],
    ["사주앱처럼 콘텐츠와 결제가 필요한 앱도 개발 상담이 가능한가요?","사주앱 아이디어의 개발 범위도 상담할 수 있습니다. 기존에 안내하는 앱 화면, 회원, 콘텐츠, 결제, 알림, 관리자, 서버·DB/API 중 필요한 기능을 조합해 범위를 정합니다. 사주 해석 방식과 사용할 콘텐츠, 자료의 출처·사용 권한을 준비해 주시면 구현에 필요한 조건을 함께 확인할 수 있습니다."],
    ["디지털노마드가 운영할 웹·앱 사업은 어떤 기능부터 상담하면 되나요?","먼저 서비스의 이용자와 판매할 콘텐츠 또는 상품, 회원·결제·예약·알림 중 필요한 흐름을 정리해 주세요. NERO는 사용자 화면과 운영 관리자, 서버·DB/API를 함께 설계하고 배포·유지보수 범위를 상담합니다. 초기 검증용 MVP로 시작할지 운영형 서비스로 구축할지는 기능과 운영 조건을 확인한 뒤 결정합니다."],
    ["외주개발 상담 전에 어떤 자료를 준비하면 좋나요?","목표 사용자, 해결하려는 문제, 꼭 필요한 기능과 우선순위, 참고 화면, 희망 예산·일정을 정리해 주세요. 기존 서비스가 있다면 현재 코드·서버·배포 계정의 보유 상태와 오류 현황을 함께 알려주시면 범위를 확인하는 데 도움이 됩니다. 기획서가 완성되지 않았더라도 상담을 통해 필요한 자료부터 정리할 수 있습니다."],
];
