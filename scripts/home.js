...
const faqs = [
    ...
    ["정부지원사업이나 모두의창업을 준비할 때 MVP 제작을 의뢰할 수 있나요?","개발 가능합니다. NERO는 지원사업과 초기 검증에 필요한 MVP/PoC, 관리자, 전환 측정, 웹·앱 배포 범위를 함께 정리합니다."],
    ...
    ["앱제작을 맡기면 서버와 관리자, iOS·Android 배포도 포함되나요?","앱 화면뿐 아니라 서버, DB/API, 관리자, 앱 빌드, 스토어 등록 자료와 심사 대응까지 함께 상담할 수 있습니다. NERO는 iOS·Android 앱서비스와 운영형 웹서비스를 개발 범위로 안내하고 있습니다. 실제 포함 항목은 프로젝트별로 정하며 스토어 승인이나 심사 기간은 서비스에 따라 달라질 수 있습니다."],
    ["앱외주개발 비용과 일정은 어떤 기준으로 정하나요?","사용자 역할과 화면, 회원·결제·알림 같은 기능, 관리자 업무, 외부 API, 검수·배포 범위를 먼저 확인합니다. NERO는 요구사항과 기능정의서, 과업범위서를 정리한 뒤 우선순위에 따라 견적을 확정합니다."],
