...
운영 배포·DNS 변경·검색엔진 등록/제출·IndexNow 전송·운영 문의 실제 발송은 자동 실행하지 않는다. Netlify 공개 경계 보강까지 포함한 최종 로컬 검사는 Node 59/59, 브라우저 28/28(45.3초), Python 9/9, robots 1/1 및 check·GA4·deploy:check 통과로 보고되었다. 상세 결과와 한계는 `completion-report.md`에서 확인한다. 운영 환경을 확인했다는 뜻은 아니다.

배포 전 [공개 파일 경계와 배포 순서](deployment-boundary.md)를 확인한다. 새 자산 목록·검사 스크립트·로컬 Netlify 플러그인도 저장소에 포함해야 한다. Git 연결 빌드로 `dist`만 게시하며 저장소 전체 폴더를 수동 업로드하지 않는다. 로컬 플러그인 경로·실패 검사는 통과했지만 실제 Netlify 원격 실행·Functions 번들·CDN 응답은 소유자가 배포 후 확인한다.
